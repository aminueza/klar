...
The simplest way is to download the latest release (for OSX and Linux) from https://github.com/aminueza/klar/releases/ and put the binary in a folder in your `PATH` (make sure it has execute permission).
...
Then run
    ...
    go get github.com/aminueza/klar
...
Klar can be dockerized. Go to `$GOPATH/src/github.com/aminueza/klar` and build Klar in project root. If you are on Linux:
